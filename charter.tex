...
El proyecto gestiona datos personales y sensibles, por lo que su tratamiento exige el cumplimiento estricto de la normativa de protección de datos vigente.

\subsection{Tipología de los datos}
Los datos que se recolectan se clasifican de la siguiente manera:
\begin{itemize}
    \item \textbf{Datos personales:} videos con la imagen del ciclista, sus medidas antropométricas y las métricas de rendimiento (potencia, cadencia) que se asocian a él.
    \item \textbf{Datos sensibles:} la frecuencia cardíaca (FC), por ser un dato relativo a la salud. Su tratamiento requiere un nivel superior de protección.
\end{itemize}

\subsection{Marco regulatorio}
El marco legal aplicable al proyecto es:
\begin{itemize}
    \item \textbf{Normativa principal:} la ley 25.326 de Protección de Datos Personales de la República Argentina y sus normativas complementarias.
    \item \textbf{Estándar de referencia:} se adoptan como buena práctica los principios del Reglamento General de Protección de Datos (RGPD) de la Unión Europea, en especial la privacidad desde el diseño.
\end{itemize}

\subsection{Requisitos de cumplimiento}
Para asegurar la legalidad y la ética del proyecto, se establecen los siguientes requisitos obligatorios:
\begin{itemize}
    \item \textbf{Consentimiento explícito:} se debe obtener el consentimiento informado y expreso de cada ciclista antes de la recolección de datos. El documento de consentimiento debe detallar con claridad la finalidad del tratamiento, el tipo de datos que se usan y los derechos del titular.
    \item \textbf{Limitación de la finalidad:} el uso de los datos se restringe exclusivamente al objetivo del proyecto: la optimización biomecánica del ciclista.
    \item \textbf{Seguridad y confidencialidad:} es imperativa la implementación de medidas técnicas y organizativas para garantizar la seguridad de los datos. Se prioriza la \textbf{anonimización} de los datos en las etapas tempranas del proceso para mitigar riesgos.
\end{itemize}
